...
<div style="text-align: center;">
	...
	Cycle Starts: <?= $begDate ?> - Cycle Ends: <?= $endDate ?>
</div>

	<form>
		<table>
		<tr>
			<td>Vendor</td><td>Count Printed</td><td>Count Completed</td><td>Responsible</td>
		</tr>		
		...
		<tr>
			...
			<?php
				...
				if($vendors[$i]['vendor_printCount'] >= $begDate && $vendors[$i]['vendor_printCount'] <= $endDate) $style = "";
			?>
			<td <?= $style ?>><?= $vendors[$i]['vendor_printCount'] ?></td>
			<?php
				$style = "style=\"background-color: coral;\"";
				if($vendors[$i]['vendor_lastCount'] >= $begDate && $vendors[$i]['vendor_lastCount'] <= $endDate) $style = "";
			?>
